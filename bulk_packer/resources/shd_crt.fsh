...
vec3 get_mask_color() {
	...
	vec2 mask_uv = mod(TexCoords, 1.0 / num_triads) * num_triads * repeat;
	vec2 slot_uv = TexCoords * u_output_size / vec2(u_mask_tex_width * 2.0 * repeat, 8.0) / u_mask_scale;
...
vec4 get_screen_geometry() {
	...
	// Calculate a scaling factor between the output vertical TexCoords and the game's, using int scale if desired
    ...
    vec2 uv = ((TexCoords - 0.5) * vertical_scale * u_zoom) + 0.5;
...
vec3 glow(vec2 uv, vec3 mask_color) {
	...
	// Diagonals
	glow += texture2D( texture0, uv + vec2( -h.x, -h.y)) * 2.0 ;
	glow += texture2D( texture0, uv + vec2( -h.x,  h.y)) * 2.0;
	glow += texture2D( texture0, uv + vec2(  h.x, -h.y)) * 2.0;
	glow += texture2D( texture0, uv + vec2(  h.x,  h.y)) * 2.0;
	// Cardinals
	glow += texture2D( texture0, uv + vec2( 0.0, -c.y));
	glow += texture2D( texture0, uv + vec2( 0.0,  c.y));
	glow += texture2D( texture0, uv + vec2(  -c.x, 0.0));
	glow += texture2D( texture0, uv + vec2(  c.x,  0.0));
...
void main() {
	...
	vec3 color = get_input_color( texture0, uv).xyz;
